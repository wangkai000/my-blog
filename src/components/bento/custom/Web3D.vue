...
// THREE.js实例
let base, rainFloor, rain, composer, controls
...
async function initScene() {
  ...
  base.renderer.outputColorSpace = THREE.LinearSRGBColorSpace
  base.renderer.useLegacyLights = true

  // 相机设置
  base.camera.position.set(0, 2, 9)
  const lookAt = new THREE.Vector3(0, 2, 0)
  base.camera.lookAt(lookAt)

  // 控制器
  controls = new kokomi.OrbitControls(base)
  ...
  const am = new kokomi.AssetManager(base, assetList)
  await am.loadAll()
  assetsLoaded.value = true

  // 灯光设置
  const pointLight1 = new THREE.PointLight('#ef77eb', 0.5, 17, 0.8)
  pointLight1.position.set(0, 2, 0)
  base.scene.add(pointLight1)

  const pointLight2 = new THREE.PointLight('#81C8F2', 2, 30)
  pointLight2.position.set(0, 25, 0)
  base.scene.add(pointLight2)
  ...
  const createText = async () => {
    ...
    const t3d = new kokomi.Text3D(base, 'love', font, {
      size: 3,
      height: 0.2,
      curveSegments: 120,
      bevelEnabled: false,
    ...
    const tm = new THREE.Mesh(
      t3d.mesh.geometry,
      new THREE.MeshBasicMaterial({ color: '#ef77eb' }),
    )
    tm.position.y = 1.54
    base.scene.add(tm)
  }
  ...
  // 创建雨效果
  rainFloor = new RainFloor(base, { count: 1000 })
  rainFloor.addExisting()
  rain = new Rain(base, { count: 1000, speed: 1.5, debug: false })
  rain.addExisting()

  // 后期处理
  composer = new POSTPROCESSING.EffectComposer(base.renderer)
  composer.addPass(new POSTPROCESSING.RenderPass(base.scene, base.camera))
  ...
  composer.addPass(new POSTPROCESSING.EffectPass(base.camera, bloom))

  const smaa = new POSTPROCESSING.SMAAEffect()
  composer.addPass(new POSTPROCESSING.EffectPass(base.camera, smaa))
...
<template>
  <div class="scene-container">
    <div id="sketch" ref="container" class="sketch-canvas" />
    <div
      class="fixed z-5 top-0 left-0 loader-screen w-screen h-screen transition-all duration-300 bg-black"
      :class="{ hollow: assetsLoaded }"
    >
      <div class="absolute hv-center">
        <div class="loading text-white text-3xl tracking-widest">
          <span v-for="(char, i) in 'LOADING'" :key="i" :style="`--i: ${i}`">{{ char }}</span>
        </div>
      </div>
    </div>
  </div>
...
.loading span {
  ...
  animation: loading 1s calc(var(--i) * 0.1s) infinite alternate;
}
